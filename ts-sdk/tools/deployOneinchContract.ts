...
async function main() {
  ...
  const HtlcFactory = await ethers.getContractFactory("HtlcUnifiedWith1inch");
  const htlc = await HtlcFactory.deploy();
  await htlc.waitForDeployment();

  const contractAddress = await htlc.getAddress();
  console.log("✅ HtlcUnifiedWith1inch deployed to:", contractAddress);

  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const abi = HtlcFactory.interface.formatJson();

  const deploymentInfo = {
    contractName: "HtlcUnifiedWith1inch",
